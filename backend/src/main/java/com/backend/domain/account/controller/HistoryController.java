...
import com.backend.domain.account.dto.CategoryListResponseDto;
...
public class HistoryController {
    ...
    @GetMapping("/category")
    @ApiOperation(value = "카테고리 조회", notes = "사용자별 카테고리 목록을 조회하는 api입니다.")
    public ResponseEntity<?> getCategoryList(
            @ApiParam(value="로그인 유저 아이디 정보", required = true) String userId) {

        CategoryListResponseDto categoryList = historyService.getCategoryList(userId);

        return success(SuccessCode.CATEGORY_LIST_RETRIEVED, categoryList);
    }
}
